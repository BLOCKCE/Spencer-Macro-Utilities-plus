...
bool BlockFirewallRule(const std::wstring& exePath) {
	...
	cmd << L"netsh advfirewall firewall add rule "
		<< L"name=\"" << FIREWALL_RULE_NAME << L"\" "
...
static void Speedglitchloop()
{
	...
	int last_fps = 0;
    while (true) {
        if (last_fps != RobloxFPS) {
            float delay_float = 1000.0f / RobloxFPS;
...
static void WallWalkLoop()
{
	...
	while (true) {
		while (!iswallwalkloop) {
			std::this_thread::sleep_for(std::chrono::milliseconds(1));
			delay = static_cast<unsigned int>(((1000.0f / RobloxFPS) + .5) * 1.1);
		}
...
{
    // Sadly, anything below 5 seconds goes back to 5 seconds
    DWORD timeout = 5000;
...
void LoadSettings(const std::string& filepath, const std::string& profile_name) {
    ...
    if (!file.is_open()) {
        // std::cerr << "Info: Settings file '" << filepath << "' not found. Using default values." << std::endl;
        return;
    }
        ...
        if (section_amounts > 0) {
            ...
            if (settings_to_load.contains("section_order_vector") && settings_to_load["section_order_vector"].is_array()) {
				...
				if (std::find(order.begin(), order.end(), 14) == order.end() && order.size() >= 1) {
					if (order.size() >= 2) {
						order.insert(order.begin() + 2, 14); // Insert at index 6 (7th element)
					}
				}
		...
		if (currentTime >= nextFrameTime) {
			...
			if (selected_section >= 0 && selected_section < sections.size()) {
				...
				if (selected_section == 6) { // Wallhop
					...
					ImGui::SameLine();
					ImGui::SetNextItemWidth(70.0f);
					if (ImGui::InputText("##", WallhopPixels, sizeof(WallhopPixels), ImGuiInputTextFlags_CharsDecimal | ImGuiInputTextFlags_CharsNoBlank)) {
						try {
				...
				if (selected_section == 8) { // Item Clip
					...
					if (ImGui::InputText("", ItemClipSlot, sizeof(ItemClipSlot), ImGuiInputTextFlags_CharsDecimal | ImGuiInputTextFlags_CharsNoBlank)) {
						try {
					...
					if (ImGui::InputText("##", ItemClipDelay, sizeof(ItemClipDelay), ImGuiInputTextFlags_CharsDecimal | ImGuiInputTextFlags_CharsNoBlank)) {
						try {
				...
				if (selected_section == 13) { // Bunnyhop
					...
					ImGui::SameLine();
					ImGui::SetNextItemWidth(120.0f);
					if (ImGui::InputText("##", BunnyHopDelayChar, sizeof(BunnyHopDelayChar), ImGuiInputTextFlags_CharsDecimal | ImGuiInputTextFlags_CharsNoBlank)) {
						try {
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow)
{
	...
	while (!done) {
    {
		...
		printf("Elapsed time: %lld s\n", static_cast<long long>(std::chrono::duration_cast<std::chrono::seconds>(now - lastPressTime).count()));
